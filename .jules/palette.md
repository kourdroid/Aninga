...
## 2026-07-18 - Keyboard Action Accessibility for CustomTkinter Widgets
...
**Action:** For full accessibility, manually bind these keys to invoke the widget's action (e.g., via `_command()` for buttons or `invoke()` for radio buttons), while defensively checking that the button is not disabled and the command exists to prevent bugs.
