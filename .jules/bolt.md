...
## 2024-07-30 - Connection Pooling for Bulk Image Downloads
**Learning:** Aninga downloads multiple manga images sequentially from the same host. Using requests.get() inside the loop creates a new TCP/SSL connection for every image, causing a massive performance bottleneck due to redundant handshake overhead.
**Action:** Always wrap iterative HTTP requests to the same host in a requests.Session() context manager and use session.get() to enable connection pooling.
