...
## 2024-06-10 - Independent Requests vs Connection Pooling
**Learning:** Using `requests.get()` inside a loop for downloading multiple images/videos from the same server creates significant TCP/SSL handshake overhead for every single file.
**Action:** Always instantiate and use `with requests.Session() as session:` to enable HTTP connection pooling, reusing connections for multiple downloads.
