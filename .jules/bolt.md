...
## 2024-07-19 - Connection Pooling for Repeated Requests
**Learning:** Using requests.get in loops forces redundant TCP/SSL handshakes per request, severely degrading download performance when scraping large volumes of images from a single CDN or domain.
**Action:** Wrap such iterative download loops in a with requests.Session() as session: context manager and use session.get() to enable connection pooling and achieve measurable speedups.
