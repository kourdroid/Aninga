...
## 2024-05-18 - requests Connection Pooling
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, causing repetitive TCP/SSL handshake overhead when downloading multiple files from the same host.
**Action:** Always instantiate and use `requests.Session()` (using a `with` context manager for proper socket cleanup) when making multiple requests to the same host to improve download performance.
