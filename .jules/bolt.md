...
## 2024-06-29 - Connection Pooling in Requests
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. Always instantiate and use `requests.Session()` when making multiple requests to the same host to eliminate repetitive TCP/SSL handshake overhead.
**Action:** Wrap iterative `requests.get` calls in a `with requests.Session() as session:` block to reuse the underlying TCP connections, drastically speeding up multiple subsequent requests to the same host.
