...
## 2026-06-05 - Missing Connection Pooling in Loops
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. This causes massive overhead due to repetitive TCP/SSL handshakes when downloading multiple files from the same host in loops.
