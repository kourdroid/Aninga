...
## 2024-05-24 - Optimize repetitive requests with connection pooling
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, causing unnecessary TCP/SSL handshake overhead.
**Action:** Always instantiate and use `requests.Session()` with a context manager when making multiple network requests.
