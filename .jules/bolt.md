...
## 2024-06-26 - TCP/SSL Handshake Overhead in Repeated Requests
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, which introduces significant TCP and SSL handshake overhead when making multiple sequential downloads to the same host in loops.
**Action:** Always instantiate and use `requests.Session()` using a `with` context manager to wrap repetitive request loops. This ensures proper socket cleanup and eliminates redundant handshake overhead for subsequent requests to the same host.
