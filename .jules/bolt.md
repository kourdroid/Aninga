...
## 2024-05-18 - Connection Pooling Anti-Pattern
**Learning:** Making multiple iterative HTTP requests to the same host using `requests.get` inside a loop incurs significant performance overhead due to redundant TCP and SSL/TLS handshakes for every file downloaded (especially noticeable when scraping manga chapters with many images).
**Action:** Always wrap loops that make repeated HTTP requests to the same origin in a `with requests.Session() as session:` context manager and use `session.get()` to enable connection pooling.
