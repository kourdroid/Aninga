...
## 2024-07-03 - Requests Session Connection Pooling
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. Sequential downloads from the same host suffer significant performance penalties due to repetitive TCP/SSL handshake overhead.
