...
## 2026-06-03 - Requests connection pooling anti-pattern
**Learning:** The requests library does not automatically pool connections across independent `get()` calls, resulting in repetitive TCP/SSL handshake overhead when downloading multiple images or episodes.
**Action:** Use `with requests.Session() as session:` and `session.get()` to eliminate overhead when making multiple requests to the same host.
