...
## 2026-07-02 - Connection Pooling
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. Making multiple requests to the same host in a loop causes repetitive TCP/SSL handshake overhead.
**Action:** Always instantiate and use `requests.Session()` (using a `with` context manager for proper socket cleanup) when making multiple requests to eliminate handshake overhead.
