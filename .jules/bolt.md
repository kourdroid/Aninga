...
## 2026-05-21 - Requests Connection Pooling Optimization
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, which causes unnecessary overhead from repetitive TCP/SSL handshakes when making multiple requests to the same host.
**Action:** Always instantiate and use `requests.Session()` when downloading multiple images/videos from the same server in loops to reuse connections and boost download performance.
