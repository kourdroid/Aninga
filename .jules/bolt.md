...
## 2026-05-26 - Connection Pooling with requests.Session()
**Learning:** Independent requests.get() calls lack connection pooling, causing massive overhead for multiple sequential downloads.
**Action:** Always instantiate and use requests.Session() using a with context manager when making multiple requests to the same host.
