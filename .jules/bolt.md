...
## 2024-06-06 - Connection Pooling with Requests Session
**Learning:** Python's requests library does not use connection pooling for independent get() calls, which creates significant repetitive TCP/SSL handshake overhead when making multiple requests to the same host.
**Action:** Always instantiate and use requests.Session() (using a with context manager for proper socket cleanup and to prevent ResourceWarnings) when making multiple requests to the same host to eliminate repetitive TCP/SSL handshake overhead.
