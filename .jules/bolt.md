...
## 2024-07-14 - Requests Connection Pooling
**Learning:** Making multiple iterative HTTP requests to the same host using Python's `requests` library without a session incurs significant overhead due to redundant TCP/SSL handshakes for each request.
**Action:** Wrap iterative HTTP requests in a `with requests.Session() as session:` context manager and use `session.get()` to enable connection pooling and improve download speeds.
