...
## 2024-07-06 - Connection Pooling in Requests
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, leading to repetitive TCP/SSL handshake overhead.
**Action:** Always instantiate and use `requests.Session()` with a context manager when making multiple requests to the same host to eliminate overhead and prevent ResourceWarnings.
