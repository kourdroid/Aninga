...
## 2024-06-07 - Connection Pooling for Downloads
**Learning:** The Python `requests` library does not use connection pooling for independent `get()` calls, causing significant TCP/SSL handshake overhead when downloading multiple images or videos from the same host.
**Action:** Always instantiate and use `requests.Session()` within a `with` block when making multiple requests to the same host.
