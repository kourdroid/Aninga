...
## 2024-05-24 - Connection Pooling for Iterative Downloads
**Learning:** In Aninga's downloading loops, making multiple `requests.get()` calls to the same host repeatedly establishes new TCP connections and SSL handshakes, drastically slowing down large batch downloads (like images in manga pages).
**Action:** Always wrap iterative `requests` operations in a `with requests.Session() as session:` block and use `session.get()` to enable connection pooling.
