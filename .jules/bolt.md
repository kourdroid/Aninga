...
## 2024-06-12 - Missing Connection Pooling
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. Making multiple requests to the same host in a loop incurs repetitive TCP/SSL handshake overhead.
**Action:** Always instantiate and use `requests.Session()` (using a `with` context manager) when making multiple requests to the same host to eliminate this overhead.
