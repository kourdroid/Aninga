...
## 2024-06-08 - Connection Pooling Missing for Downloads
**Learning:** Making independent `requests.get()` calls in loops for downloading manga images or anime episodes does not utilize connection pooling, leading to repetitive TCP/SSL handshake overhead for the same host.
**Action:** Always instantiate and use `requests.Session()` within a `with` context manager when making multiple requests to the same host to enable connection pooling and improve download performance.
