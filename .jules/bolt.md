...
## 2024-07-05 - Requests Session TCP/SSL Handshake Overhead
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls, which introduces significant TCP/SSL handshake latency when repeatedly downloading numerous small resources (like manga images) from the same host.
**Action:** Always wrap repetitive `requests.get()` operations within a `requests.Session()` context manager to reuse connections, significantly improving download performance and preventing resource leaks.
