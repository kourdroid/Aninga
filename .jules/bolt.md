...
## 2024-06-23 - Independent requests.get() calls in loops
**Learning:** Python's `requests` library does not use connection pooling for independent `get()` calls. Making multiple independent requests to the same host in a loop (like downloading multiple manga pages) causes repetitive TCP/SSL handshake overhead, creating a significant performance bottleneck.
**Action:** Always instantiate and use `requests.Session()` (using a `with` context manager) when making multiple requests to the same host to eliminate connection overhead and speed up bulk downloads.
