...
## 2024-05-19 - Connection Pooling for Repeated Requests
**Learning:** Making multiple iterative HTTP requests to the same host using Python's `requests` library without a session incurs significant overhead due to redundant TCP/SSL handshakes.
**Action:** Always wrap repeated calls in a `with requests.Session() as session:` context manager and use `session.get()` to enable connection pooling and improve performance.
