...
## 2026-05-01 - Keyboard Accessibility for CTkEntry Widgets
**Learning:** For CustomTkinter `CTkEntry` widgets lacking visual focus (e.g., those with `border_width=0`), keyboard users receive no visual feedback indicating which input is currently focused, violating basic accessibility guidelines.
**Action:** Improve accessibility by setting a default border width that matches the background color and binding `<FocusIn>` and `<FocusOut>` events to toggle the `border_color`.
