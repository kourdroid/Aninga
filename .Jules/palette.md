...
## 2024-05-19 - Visual Focus States for CustomTkinter Zero-Border Entries
**Learning:** `CTkEntry` widgets in CustomTkinter that have `border_width=0` lack any visual indicator when they receive keyboard focus, making the application difficult to navigate for keyboard-only users.
**Action:** When a borderless entry design is desired, set `border_width` to a small value (e.g., 2) but set `border_color` to match the entry's background color so it appears borderless when unfocused. Then, bind `<FocusIn>` and `<FocusOut>` events to toggle the `border_color` to a primary/accent color when focused.
