...
## 2024-05-11 - CTkEntry Focus Visibility Fix
**Learning:** CustomTkinter `CTkEntry` widgets with `border_width=0` lack any visual state change on focus, which is a major accessibility issue for keyboard navigation and user feedback.
**Action:** When creating borderless CTkEntry widgets, explicitly add focus styles by setting a default border matching the background and dynamically updating it on `<FocusIn>`/`<FocusOut>` using safely bound lambda functions to ensure accessible UI interaction.
