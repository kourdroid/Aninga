...
## 2026-05-03 - Focus indicators for borderless CustomTkinter Entries
**Learning:** `CTkEntry` widgets with `border_width=0` lack visual focus indicators, making it difficult for keyboard users and general users to know which input is currently active.
**Action:** Give borderless entries a default `border_width` matching their background color, and use `bind('<FocusIn>')` and `bind('<FocusOut>')` to toggle the `border_color` to a primary/accent color.
