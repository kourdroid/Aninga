...
## 2024-05-19 - CustomTkinter Keyboard Focus Indicators
**Learning:** By default, CustomTkinter `CTkEntry` inputs with `border_width=0` provide no visual feedback when they receive keyboard focus, violating accessibility standards for keyboard navigation.
**Action:** Always ensure interactive elements like inputs have a visible focus state. For `CTkEntry`, this can be implemented by setting a default border width with a color matching the background, and binding `<FocusIn>` and `<FocusOut>` events to toggle the border color to a primary highlight color.
